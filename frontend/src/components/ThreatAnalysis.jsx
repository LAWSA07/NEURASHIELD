...
const ThreatAnalysis = () => {
  ...
  useEffect(() => {
    const fetchThreatData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:8001/alerts/${threatId}`);
        if (!response.ok) {
        ...
        const relatedResponse = await fetch(`http://localhost:8001/alerts?limit=5`);
  ...
  const handleUpdateStatus = async (newStatus) => {
    ...
    try {
      const response = await fetch(`http://localhost:8001/alerts/${threatId}/status`, {
        method: 'PUT',
        headers: {
